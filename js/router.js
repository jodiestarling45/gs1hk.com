...
function navigate(event) {
    ...
    if (!routeInfo) {
        ...
        var x = document.getElementById(routeInfo.id);
        var $contents = $('.tab-content');
        console.log(routeInfo)
        $contents.slice(1).hide();
        $('.tab').click(function () {
            var $target = $('#' + routeInfo.id).show();
            $contents.not($target).hide();

        });
        $(document).ready(function () {
            var $target = $('#').css("border-top","5px solid orangered");
        });
    }
...
var myFirstRouter = new router('myFirstRouter', [
    ...
    {
        path: '/toAndFormHongkong',
        name: 'ToAndFromHongKong',
        id: 'ToAndFromHongKong'
    }
])

var currentPath = window.location.pathname;
console.log(currentPath);
if (currentPath === '/deepdive/') {
    console.log("this is the oveview page ")
    view.innerHTML = "the  is the overview page";
} else {
    ...
    console.log(route);
    if (route) {
        view.innerHTML = "you are on the " + route.path;
    } else {
        view.innerHTML = "404";
    }
}
...
(function () {
    var h = {}, mt = {}, c = {
        id: "93752d2a950a63a79de398675997e395",
        dm: ["https://www.gs1hk.org/", "gs1hk.org"],
        js: "tongji.baidu.com/hm-web/js/",
        etrk: [],
        cetrk: [],
        cptrk: [],
        icon: '',
        ctrk: ["%5b%22http%3a%5c%2f%5c%2fgs1hk.org%22%2c%22https%3a%5c%2f%5c%2fgs1hk.org%5c%2fzh-cn%5c%2fenquiry-form-membership%22%5d"],
        nv: 1,
        vdur: 1800000,
        age: 31536000000,
        rec: 0,
